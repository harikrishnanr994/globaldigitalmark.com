			...
			if ( ! empty( $_REQUEST['lp_orderby'] ) ) {
				...
				switch ( $orderby ) {
					...
					case 'students-asc':
						...
						if ( $orderby && $order ) {
							$course_filter = array_merge( $course_filter, array(
								'orderby' => $orderby,
								'order'   => $order
							) );
...
if ( ! function_exists( 'thim_override_demo_image_tp_chameleon' ) ) {
	function thim_override_demo_image_tp_chameleon() {
		return THIM_URI . 'images/all-demo.png';
	}
}
 ...
 * RTL Support.
 */
if ( ! function_exists( 'thim_rtl_support' ) ) {
...
if ( ! is_admin() ) {
	if ( get_theme_mod( 'thim_rtl_support', false ) ) {
		add_filter( 'language_attributes', 'thim_rtl_support' );
	}
}
                    ...
                    <div class="copyright-content">
                        <div class="row">
							...
							if ( empty( $copyright_text ) ) {
								$copyright_text = '<a href="' . esc_url( 'https://themeforest.net/item/education-wordpress-theme-education-wp/14058034' ) . '" target="_blank">Education WordPress Theme</a> by <a href="' . esc_url( 'http://www.thimpress.com' ) . '" target="_blank">ThimPress.</a> Powered by WordPress.';
							}
...
if ( ! function_exists( 'thim_footer_class' ) ) {
	function thim_footer_class() {
		...
		$footer_class    = is_active_sidebar( 'footer_bottom' ) ? $custom_class . ' has-footer-bottom' : $custom_class;
